...
int DRobotFileSystem::getSubdirectoryFileList(std::string root,
	...
	while ((dirp = readdir(dp))) {
		...
		if (S_ISDIR(filestat.st_mode)) {
			...
			if (!std::strcmp(dirp->d_name, ".")
					|| !std::strcmp(dirp->d_name, ".."))
				continue;
			...
			std::cerr
					<< "Error... unexpected file found in the directory containing names. The file will be neglected."
					<< std::endl;
...
// TODO: we should probably change this method to use the boost library instead;
int DRobotFileSystem::getDirectory(std::string path, std::string& directory) {
	char str[1024];
...
// TODO: we should probably change this method to use the boost library instead;
int DRobotFileSystem::getFilename(std::string path, std::string& filename) {
	char str[1024];
	path.copy(str, path.length());
	str[path.length()] = 0;
...
bool DRobotFileSystem::fileExists(std::string path) {
...
bool DRobotFileSystem::directoryExists(std::string path) {
...
void DRobotFileSystem::createDirectory(std::string path) {
	mkdir(path.c_str(), 0777);
}

void DRobotFileSystem::removeDirectory(std::string path) {
	std::string command = "rm -rf " + path;
	system(command.c_str());
}

std::vector<std::string> DRobotFileSystem::getFileList(std::string path) {
...
int DRobotFileSystem::getFileCount(std::string path) {
	return getFileList(path).size();
}

std::vector<std::string> DRobotFileSystem::getDirectoryList(std::string path) {
...
int DRobotFileSystem::getDirectoryCount(std::string path) {
	return getDirectoryList(path).size();
}

std::string DRobotFileSystem::getDirectoryName(std::string path) {
...
std::string DRobotFileSystem::getFileName(std::string path) {
